...
class Tester:
    ...
    def run_on_image_folder(self, all_image_folder, detections, output_folder, visualize_proj=True):
        ### 추가 코드
        # txt에 저장되어 있는 CoTracker tracking 좌표를 tensor로 변환
        # loaded_list = []
        # txt_path = '/content/drive/MyDrive/Colab Notebooks/cv_student/BEDLAM/CoTracker'
        # with open(f'{txt_path}/tensor_values.txt', 'r') as file:
        #     for line in file:
        #         line = line.replace('[', '').replace(']', '').split(', ')
        #         row_values = list(map(float, line))
        #         cnt = 0
        #         for idx, val in enumerate(row_values):
        #             if cnt == 4:
        #                 loaded_list.append(lst1)
        #                 cnt = 0
        #             if idx % 2 == 0: 
        #                 if cnt == 0:
        #                     lst1 = []
        ...
        # loaded_np_array = np.array(loaded_list)             
        # loaded_tensor = torch.tensor(loaded_np_array, device='cuda:0')
        ###
        ...
        for fold_idx, image_folder in enumerate(all_image_folder):
            ...
            for img_idx, img_fname in tqdm.tqdm(enumerate(image_file_names)):
                ...
                # 추가
                # if img_idx == 0:
                #     # print(f"테스트 tester.py -> hmr_output : {hmr_output}")
                #     joint_1 = hmr_output['joints2d'][0][10]
                #     joint_2 = hmr_output['joints2d'][0][11]
                #     print(f'변경 전 hmr_output[joints2d][0][10] = {joint_1}') # 출력결과 : hmr_output[joints2d][0][10] = tensor([368.9165, 397.5112], device='cuda:0')
                #     print(f'변경 전 hmr_output[joints2d][0][11] = {joint_2}') # 출력결과 : hmr_output[joints2d][0][10] = tensor([271.6797, 400.1758], device='cuda:0')
                    

                # ### joits2D idx 10, 11 좌표를 cotracker 좌표로 변경하여 렌더링

                # # CoTrcker 좌표로 변경    
                # hmr_output['joints2d'][0][10] = loaded_tensor[fold_idx][0] # 왼발 끝
                # hmr_output['joints2d'][0][11] = loaded_tensor[fold_idx][1] # 오른발 끝
                # if img_idx == 0:
                #     joint_1 = hmr_output['joints2d'][0][10]
                #     joint_2 = hmr_output['joints2d'][0][11]
                #     print(f'변경 후 hmr_output[joints2d][0][10] = {joint_1}') 
                #     print(f'변경 후 hmr_output[joints2d][0][11] = {joint_2}')
                ###    
                ...
                # print(f'pred_vertices_array : {pred_vertices_array}')
                # print(f'len(pred_vertices_array) : {len(pred_vertices_array[0])}')
