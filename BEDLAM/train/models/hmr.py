...
class HMR(nn.Module):
    def __init__(
        ...
        if hparams.TRIAL.version == 'real':
            # print("테스트 : hmr.py -> hparams.TRIAL.version == 'real'") ### 추가
            if hparams.TRIAL.bedlam_bbox:
                # print("테스트 : hmr.py ->  if hparams.TRIAL.bedlam_bbox") ### 추가
            ...
            if hparams.TRIAL.bedlam_bbox:
                # print("테스트 : hmr.py -> hparams.TRIAL.bedlam_bbox") ### 추가 > 사용
                self.head = HMRHeadCLIFF(
                    num_input_features=get_backbone_info(backbone)['n_output_channels'],
                    backbone=backbone,
                )
                if hparams.DATASET.proj_verts:
                    # print("테스트 : hmr.py -> hparams.DATASET.proj_verts") ### 추가
                    self.smpl = SMPLXCamHeadProj(img_res=img_res) 
                else:
                    # print("테스트 : hmr.py -> hparams.DATASET.proj_verts  else:") ### 추가 > 사용
                    self.smpl = SMPLXCamHead(img_res=img_res)

            else:
                # print("테스트 : hmr.py -> hparams.TRIAL.bedlam_bbox else:") ### 추가
    ...
    def forward(
        ...
        if self.hparams.TRIAL.bedlam_bbox:
            # print("테스트 : hmr.py -> self.hparams.TRIAL.bedlam_bbox") ### 추가 > 사용
            ...
            # print("테스트 : hmr.py -> self.hparams.TRIAL.bedlam_bbox else:") ### 추가
            smpl_output = self.smpl(
                rotmat=hmr_output['pred_pose'],
                shape=hmr_output['pred_shape'],
                cam=hmr_output['pred_cam'],
